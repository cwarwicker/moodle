...
class assign_feedback_file extends assign_feedback_plugin {
     ...
     * @return mixed
     */
    public function get_file_feedback(int $gradeid, ?int $markid = null): mixed {
        global $DB;
    ...
    public function view_summary(stdClass $grade, &$showviewlink, bool $fromgradingtable = false, ?int $markid = null) {
        global $OUTPUT;
        // Show the summary from the grading table, which might be for a specific mark, not the overall.
        if ($fromgradingtable) {
            [$filearea, $fileitemid] = $this->get_fileitem_area_id($grade, $markid);
    ...
    public function view(stdClass $grade): string {
        ...
        foreach ($filefeedbackitems as $filefeedbackitem) {
            ...
            if (is_null($filefeedbackitem->mark)) {
                ...
                $feedback['context'] = get_string('markerfiles', 'assignfeedback_file');
                $overall = false;
            }
    ...
    public function view_batch_upload_files($users) {
        ...
        $formparams['assignment'] = $this->assignment;
        ...
        if ($mform->is_cancelled()) {
            ...
            foreach ($users as $userid) {
                ...
                // If this is marker feedback, but the user is not an allocated marker for this student - skip.
                if ($data->uploadcontext === 'mark' && !$this->assignment->is_user_allocated_marker($USER->id, $userid)) {
                    continue;
                }

                // If this is marker feedback, get/create the mark for this user.
                $markid = ($data->uploadcontext === 'mark') ?
                    $this->assignment->get_mark($grade->id, $USER->id, true)->id :
                    null;

                // Get the correct filearea and itemid based on whether this is marker feedback or not.
                if (!is_null($markid)) {
                    [$filearea, $fileitemid] = $this->get_fileitem_area_id($grade, $markid);
                } else {
                    [$filearea, $fileitemid] = $this->get_fileitem_area_id($grade);
                ...
                // Update the database records showing how many files exist for this.
                $filefeedback = $this->get_file_feedback($grade->id, $markid);
                if ($filefeedback) {
                    $filefeedback->numfiles = $this->count_files($fileitemid, $filearea);
