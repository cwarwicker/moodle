...
class manager {
    ...
    public static function queue_adhoc_task(adhoc_task $task, $checkforexisting = false) {
        ...
        try {
            ...
            if ($existingid = self::revive_adhoc_task($task, $record)) {
                return $existingid;
            }
            throw $e;
        }
    ...
    /**
     * Find an existing queued adhoc task by identity hash.
     *
     * If the task exists and has exhausted its retries, revive it so it can run again.
     *
     * @param adhoc_task $task
     * @param \stdClass $record
     * @return int
     */
    private static function revive_adhoc_task(adhoc_task $task, \stdClass $record): int {
        global $DB;
        ...
        // If we get here it means the task is already queued/running/retrying.
